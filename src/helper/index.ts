...
export const getCustomHeaders = async (
    env: Env,
    customerId: string,
    endpointId: string
) => {
    ...
    const existingEndpoint: Endpoint | undefined =
        existingCustomer.endpoints.find(
            (data) => data.endpointId === endpointId
        );
...
export const getRetryConfig = async (
    env: Env,
    customerId: string,
    endpointId: string
) => {
...
export const getEndpointDetails = async (
    ...
    const endpointDetails = customerDetails.customer?.endpoints.find(
...
export const getOneCustomerDetails = async (
    env: Env,
    key: Keys
...
export const getCustomersDetails = async (
    ...
    const values = (await Promise.all(
        keys.map(async (key) => await getOneCustomerDetails(env, key))
    )) as Customer[];
    return values;
};
